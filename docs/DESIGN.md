...
Pack-Wise is flat by default. Cards, inputs, and the navbar sit at zero elevation at rest — depth is not conveyed by shadow at all in the resting state, only by the navy/border hairline and background-color contrast. Shadows exist purely as a _hover response_, and they are always tinted with the brand color rather than neutral gray, reinforcing that yellow/navy glow is the "you are being invited to act" signal.
